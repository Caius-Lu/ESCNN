...
class FPEM_FFM(nn.Module):
    def __init__(self, backbone_out_channels, inner_channels=128, fpem_repeat=1):
        """
        PANnet
        :param backbone_out_channels: 基础网络输出的维度
        """
        ...
        self.attention = NonLocalBlock(channel=self.out_channels)
    ...
    def forward(self, x):
        ...
        # cac = torch.cat([c2_ffm, c3, c4, c5], dim=1)
        # # a_tt = self.attention(cac)
        # Fy = self.conv_bn(torch.cat([a_tt,cac], dim=1))
        # print('c5: ',c5.shape)
        # print('c4: ',c4.shape)
        # print('c3: ',c3.shape)
        # print('c2_ffm: ',c2_ffm.shape)
        # print('Fy: ',Fy.shape)
        # exit()
        return c5
