...
class CriterionDSN(nn.Module):
    ...
    def forward(self, preds, batch):
        target = batch['label']
        target = torch.as_tensor(target, dtype=torch.long, device=target.get_device())  #
        h, w = target.size(1), target.size(2)
        if len(preds) >= 2:
            scale_pred = F.interpolate(input=preds[0], size=(h, w), mode='bilinear', align_corners=True)
            # print("tensor0 max value", scale_pred.max())
            # print("tensor0 min value", scale_pred.min())
            loss1 = self.criterion(scale_pred, target)

            scale_pred = F.interpolate(input=preds[1], size=(h, w), mode='bilinear', align_corners=True)
            # print("tensor1 max value", scale_pred.max())
            # print("tensor1 min value", scale_pred.min())
            loss2 = self.criterion(scale_pred, target)
        else:
            scale_pred = F.interpolate(input=preds[0], size=(h, w), mode='bilinear', align_corners=True)
            loss = self.criterion(scale_pred, target)
            return loss
        metrics = dict(loss1=loss1, loss2=loss2)
        metrics['loss'] = loss1 + loss2 * 0.4
        return metrics
...
class CriterionOhemDSN(nn.Module):
    ...
    def forward(self, preds, batch):
        target = batch['label']
        # print('target.get_device()', target[0].get_device())
        # target = torch.tensor(target, dtype=torch.long,device=target.get_device())#
        target = torch.as_tensor(target, dtype=torch.long,device=target.get_device())#
        h, w = target.size(1), target.size(2)
